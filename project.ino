...
void readFingerInput() {
  if (state == LOCK_STATE && finger.getFingerprintIDez() != 1) {
    buzzer.success();
    lcd.display("Correct password", "Welcome back", 1);
    state = UNLOCK_STATE;
  } else if (state == LOCK_STATE && finger.getFingerprintIDez() == 1) {
    fpAttempt++;
  } else if (state == ADD_FINGERPRINT_STATE && !finger.isFirstImage() && finger.getFirstImage() == 1) {
    buzzer.beep();
    lcd.display("Remove finger");
    delay(2000);
    lcd.display("Put your SAME finger again");
  } else if (state == ADD_FINGERPRINT_STATE && finger.isFirstImage() && finger.addFinger()) {
    buzzer.success();
    lcd.display("Change password successfully");
    state = UNLOCK_STATE;
  }
}
...
void addFingerprintState() {
  buzzer.beep();
  lcd.display("Put your finger for new user");
}
